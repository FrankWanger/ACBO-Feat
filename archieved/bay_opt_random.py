...
for surrogate in Surrogates:
    ...
    for trial in range(1,num_trial+1):
        ...
        for iter in tqdm(range(1,num_iter+1)):
            ...
            new_rand_index = random.randint(0, len(y_candidate) - 1)
            new_x = X_candidate[new_rand_index]
            new_y = y_candidate[new_rand_index]

            # Add the new point to the training set
            #my_surrogate.add_data(new_x, new_y)

            # Remove the new point from the candidate set
            X_candidate = np.delete(X_candidate, new_rand_index, axis=0)
            y_candidate = np.delete(y_candidate, new_rand_index)
            ...
            mol_added = np.append(mol_added, mol_track_candidate[new_rand_index])
    ...
    np.save(f'results/lipo_{featurizer_name}_ratio{partition_ratio}_iter{num_iter}_trial{num_trial}'+str(surrogate)+'_random.npy', results)
    #bestx, besty, hps, iter, bestobservedylabel
    torch.save(my_surrogate, f'results/model_{featurizer_name}_ratio{partition_ratio}_iter{num_iter}_trial{num_trial}'+str(surrogate)+'_random.pickle')
